                            ...
                            @foreach ($transfers as $transfer)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @elseif($transfer->hod_transfer_approve=="1")
                                        <div class="badge badge-pill badge-success">Approve</div>
                                        @else($transfer->hod_transfer_approve=="2")
                                        <div class="badge badge-pill badge-danger">Reject</div>
                                        @endif
                                    </td>
                                    <td>
                                         @if($transfer->hr_transfer_approve=="0")
                                        <div class="badge badge-pill badge-warning">Pending</div>
                                        @elseif($transfer->hr_transfer_approve=="1")
                                        <div class="badge badge-pill badge-success">Approve</div>
                                        @else($transfer->hr_transfer_approve=="2")
                                        <div class="badge badge-pill badge-danger">Reject</div>
                                        @endif
                                    </td>
                                    @if(Gate::check('Edit Transfer') || Gate::check('Delete Transfer'))
                                        <td>
                                             @role('company||hr')
                                            <a href="#" data-url="{{ route('transfer.show',$transfer->id) }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Transfer Detail')}}" class="edit-icon bg-success" data-toggle="tooltip" data-original-title="{{__('View Detail')}}"><i class="fas fa-eye"></i></a>
                                            @endrole
                                            @can('Edit Transfer')
