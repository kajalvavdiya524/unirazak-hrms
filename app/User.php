...
class User extends Authenticatable
{
    ...
    public function employeeIdFormat($number, $designation)
    {
        $settings = Utility::settings();
        $words = explode(" ", $designation);
        $acronym = "";
        $acronym =substr($words[0], 0, 3);
        // foreach ($words[0] as $w) {
        //     // $acronym .= $w[];
        // }
         return   '#'.strtoupper($acronym).sprintf("%05d", $number);
    ...
    public function getCountLeave($userid,$leave_type)
    {
        $leave= Leave::where('employee_id','=',$userid)->where('leave_type_id','=',$leave_type)->get()->count();
        return $leave;
    }
