    ...
    public function changeaction(Request $request)
    {
        ...
        if(Auth::user()->type == 'hr')
        {
            $leave->status = $request->status;
                ...
                {
                    $st='Approval';
                    $leave->hr_leave_approval=1;
                    $notify->body="Hr Approve Leave request";
                }
                elseif($request->status=='Reject')
                {
                    $st='Reject';
                    $leave->hr_leave_approval=2;
                    $notify->body="Hr Reject Leave request";
                }
                $notify->from_id=Auth::user()->id;
                $notify->to_id=$leave->employee_id;
        ...
        if(Auth::user()->type == 'company' || Auth::user()->type == 'super admin')
        {
            ...
            {
                $st='Approval';
                $leave->hod_leave_approval=1;
                $notify->body="HOD Approve Leave request";
            }
            elseif($request->status=='Reject')
            {
                $st='Reject';
                $leave->hod_leave_approval=2;
                $notify->body="HOD Reject Leave request";
            }
            $notify->from_id=Auth::user()->id;
            $notify->to_id=$leave->employee_id;
        }
